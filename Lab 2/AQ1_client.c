...
void* tcp_client(void *arg) {
    ...
    printf("Local Socket Address (Client side):\n");
...
void* udp_client(void *arg) {
    ...
    // Get local socket information (for UDP, getsockname may show 0.0.0.0 until first send)
    local_addr_len = sizeof(local_addr);
    getsockname(sock, (struct sockaddr *)&local_addr, &local_addr_len);

    printf("Local Socket Address (before send):\n");
    printf("  IP Address: %s\n", inet_ntoa(local_addr.sin_addr));
    printf("  Port Number: %d\n", ntohs(local_addr.sin_port));
    ...
    printf("Local Socket Address (after send):\n");
    printf("  IP Address: %s\n", inet_ntoa(local_addr.sin_addr));
    printf("  Port Number: %d\n", ntohs(local_addr.sin_port));
...
int main() {
    ...
    printf("=== Combined TCP/UDP Client ===\n");
    printf("Starting both TCP and UDP clients...\n");
